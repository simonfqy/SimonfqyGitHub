...
bool BoundingBox::is_intersected(const BoundingBox& rhs) const { 
	...
	{
		cerr << "dimensionality inconsistency" << endl;
	}
...
int BoundingBox::enlargement(const BoundingBox& rhs){
	BoundingBox another = BoundingBox(*this);
	return (another.group_with(rhs).get_area() - this->get_area());
}
