...
class Solution:
    ...
    def partition(self, nums, start, end, k):
        ...
        pivot = nums[(start + end) // 2]
        while left <= right:
            # Here we have to use < or > pivot, cannot use <= or >= pivot. Otherwise it will encounter infinite function calls.
            while left <= right and nums[left] < pivot:
                left += 1
            while left <= right and nums[right] > pivot:
                right -= 1
        ...
        # left is now bigger than right. Here we have to use k <= and k >=, cannot drop the equal sign. Otherwise it produces wrong output.
        if k <= right:
